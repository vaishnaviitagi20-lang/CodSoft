...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section className="py-16 bg-white dark:bg-ink-950 transition-colors">
        <div className="page-container">
          <div className="text-center mb-10">
            ...
            <p className="text-ink-400 dark:text-ink-300">Explore opportunities across 12 professional fields</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {JOB_FIELDS.slice(0, 12).map((f) => (
              <Link key={f} to={`/jobs?field=${encodeURIComponent(f)}`}
                className={`flex flex-col items-center gap-2 p-4 rounded-2xl border text-center hover:shadow-md transition-all group ${FIELD_COLORS[f] || "bg-ink-50 dark:bg-ink-900 border-ink-200 dark:border-ink-800"}`}>
      ...
      {/* Recent Jobs */}
      <section className="py-16 bg-ink-50 dark:bg-ink-900/50 transition-colors">
        <div className="page-container">
          <div className="flex items-end justify-between mb-8">
            <div>
              <h2 className="section-title mb-1">Latest Opportunities</h2>
              <p className="text-ink-400 dark:text-ink-300 text-sm">Freshly posted — updated daily</p>
            </div>
