...
export default function Register() {
  ...
  return (
    <div className="min-h-screen bg-ink-50 dark:bg-ink-900/50 flex flex-col transition-colors">
      <Navbar />
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-lg">
          <div className="text-center mb-8">
            <h1 className="font-display text-3xl font-bold text-ink-900 dark:text-white mb-1">Join TalentFlow+</h1>
            <p className="text-ink-400 dark:text-ink-300">Create your free account in seconds</p>
          </div>
          ...
          <div className="card p-8">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button key={r.value} type="button" onClick={() => set("role", r.value)}
                    className={`flex flex-col items-center gap-1.5 p-4 rounded-2xl border-2 transition-all ${form.role === r.value ? "border-ink-900 dark:border-ink-500 bg-ink-50 dark:bg-ink-800/50" : "border-ink-100 dark:border-ink-800 hover:border-ink-300 dark:hover:border-ink-600 bg-white dark:bg-ink-900/50"}`}>
                    <span className="text-2xl">{r.icon}</span>
                    <span className="font-semibold text-ink-900 dark:text-white text-sm">{r.label}</span>
                    <span className="text-xs text-ink-400 dark:text-ink-500">{r.desc}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-400 dark:text-ink-500 text-sm">
                      {showPass ? "🙈" : "👁️"}
                    </button>
                  </div>
                </div>
              ...
              {form.role === "employer" && (
                <div className="space-y-3 pt-2 border-t border-ink-100 dark:border-ink-800">
                  <p className="text-sm font-semibold text-ink-700 dark:text-ink-300">Company Details</p>
                  <div>
            ...
            <p className="text-center text-sm text-ink-500 dark:text-ink-400 mt-5">
              ...
              <Link to="/login" className="text-ink-900 dark:text-white font-semibold hover:text-gold-600 transition-colors">Sign in →</Link>
            </p>
          </div>
        </div>
      </div>
